...
Press the question mark in the capsule toolbar to see this list in the app. Mac keys are shown; on Windows use `Ctrl` where `⌘` appears.

<Frame caption="The in-app shortcut reference">
  ![Shortcut help popup showing the node, editing, viewport, and tag groups side by side](/images/9月1日\(44\).gif)
</Frame>
...
<Note>
  `V` is overloaded on purpose. With nothing selected it creates a video node; with two or more nodes selected it aligns them vertically.
</Note>
